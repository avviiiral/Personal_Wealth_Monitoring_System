...
def two_factor_setup(request):
    """
    POST /api/settings/me/2fa/setup/

    Generate a new TOTP secret and return the provisioning URI.

    The secret is stored temporarily in the user's profile but
    2FA is NOT enabled until the user successfully verifies a
    TOTP code.
    """
    ...
    profile.save(update_fields=["two_factor_secret", "updated_at"])

    totp = pyotp.TOTP(secret)

    provisioning_uri = totp.provisioning_uri(
        name=request.user.email or request.user.username,
        issuer_name="Personal Wealth Monitoring System",
    )
...
def two_factor_enable(request):
    """
    POST /api/settings/me/2fa/enable/

    Verify the authenticator code and enable TOTP 2FA.
    """
...
def two_factor_disable(request):
    """
    POST /api/settings/me/2fa/disable/

    Disable TOTP 2FA.

    Requires both the current password and the current
    authenticator code.
    """
...
def verify_two_factor_login(request):
    """
    POST /api/settings/auth/2fa/verify/

    Complete the login process for a user whose password was
    correct but whose account requires TOTP two-factor authentication.

    The Django session is created ONLY after the TOTP code is valid.
    """
    ...
    # Only establish the Django session after successful TOTP verification.
