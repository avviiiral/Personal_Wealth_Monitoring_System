...
export class HoldingReportsComponent implements OnInit {
  ...
  async downloadAssetNameReport(group: SubClassGroup, holding: HoldingGroup, event?: Event): Promise<void> {
    event?.stopPropagation();
    await this.downloadRows([this.toExportRow(holding.row, holding.row.asset_name_xirr, true)], this.assetNameColumns(), `holding_report_asset_${this.slugify(holding.asset_name)}_${this.todayStamp()}.xlsx`, `Asset Name Holding Report — ${holding.asset_name} (as of ${this.todayLabel()})`);
  }
