...
export class PortfolioComponent implements OnInit, OnDestroy {
  ...
  /** Independent sorting state for each Portfolio hierarchy level. */
  private readonly subclassSort: PortfolioSortState = {
    column: null,
    direction: 'normal',
  };
   ...
   * Sort cycle:
   *
   * 1st click  -> decreasing
   * 2nd click -> increasing
   * 3rd click -> normal/default
   *
   * Sorting is independent at each hierarchy level.
   */
  cycleSort(
  ...
  private sortRows<T>(
    ...
    level: 'subclass' | 'asset' | 'underlying',
    parentKey: string,
    valueGetter: (row: T, column: PortfolioSortColumn) => unknown,
  ...
  private getSortableValue(summary: SubClassSummary, column: PortfolioSortColumn): number | null {
    switch (column) {
      case 'quantity':
        return this.toNullableNumber(summary.quantity);

      case 'invested_value':
        return this.toNullableNumber(summary.invested_value);

      case 'current_value':
        return this.toNullableNumber(summary.current_value);

      case 'pnl':
        return this.toNullableNumber(summary.pnl);

      case 'xirr':
        return this.toNullableNumber(summary.xirr);
  ...
  getAssetGroups(assets: PortfolioAssetNode[], parentKey = ''): AssetGroup[] {
    ...
    const groupsList: AssetGroup[] = Array.from(groups.entries()).map(
      ([asset_name, groupedAssets]) => {
    ...
    return this.sortRows(
      ...
      (group, column) => {
        switch (column) {
          ...
          case 'invested_value':
            return group.invested_value;
          case 'current_value':
            return group.current_value;
          case 'pnl':
            return group.pnl;
          case 'xirr':
            return group.xirr;
  ...
  getSortedUnderlyingAssets(assets: PortfolioAssetNode[]): PortfolioAssetNode[] {
    ...
    return this.sortRows(
      ...
      (asset, column) => {
        switch (column) {
          case 'underlying':
            return this.getUnderlyingName(asset);
          case 'family':
            return asset.family_name || '';
          case 'isin':
            return asset.isin || '';
          case 'quantity':
            return asset.quantity;
          case 'invested':
            return asset.invested_value;
          case 'current_price':
